      ...
      <span>
        ...
        <StyleRoot style={{ width: '100%', minHeight: '100%' }}>
          ...
          <Nag onResend={_ => resend(session.user.email)} shouldVerify={session.user && session.user.verificationCode}
               shouldChangePassword={session.user && session.user.shouldChangePassword}
               noAccounts={accounts.isFetched && accounts.data.length === 0} 
               licenseUpdated={updated(members.data)} licenseMissing={missing(members.data)} />
